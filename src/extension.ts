...
export function activate(context: vscode.ExtensionContext) {
    ...
    const fileContentMonitoringService = new FileContentMonitoringService(async (document: vscode.TextDocument, newHash: string) => {
        ...
        outputChannel.appendLine(`[INFO] Document changed: ${document.uri.fsPath}`);
        ...
        try {
            ...
            // Update the stored hash after successful analysis
    ...
    }, hashStorageService, configurationService, outputChannel);

    fileContentMonitoringService.startMonitoring();

    // Add the services to subscriptions so they're disposed when the extension deactivates
